...
// inclure local_dep
for (int e = 0; e < num_machines; ++e){
  for (auto d : local_dep){ // d est de type pair
    MPConstraint* ct = solver.MakeRowConstraint(0,solver.infinity(),"");
    ct->SetCoefficient(var[d.first][e], -1.0); // d est une pair
    ct->SetCoefficient(var[d.second][e], 1.0);
  }
}
