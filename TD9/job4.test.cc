...
int main() {
  ...
  set<int> blacklist = {};
  const int kExpected[] = {0, 3,  0,  1, 1,  4, 2, 2,  4,  11,
                           8, 11, 12, 9, 19, 7, 7, 20, 19, 16};
  int num_jobs = 0;
  int num_machines = 0;
  ...
  for (int test = 0; test < num_tests; ++test) {
    if (blacklist.count(test) > 0) continue;
    num_jobs += random.Uniform(5);
    num_machines += random.Uniform(2);
    TestCase big;
    ...
    // exclu
    const int num_exclu = num_jobs < 9999 ? 0 : random.Uniform(num_jobs);
  ...
  for (const TestCase& t : test_cases) {
    cout << endl
         << "==========================================================" << endl
         << "Testing '" << t.name << "'..." << endl;
    const vector<int> sol = BestJobAssignment4(t.jobs, t.machines, t.dep);
    ...
    // Verify that the interdependency constraints are verified.
    for (int i = 0; i < t.dep.size(); ++i) {
      const int a = t.dep[i].first;
      const int b = t.dep[i].second;
      if ((sol[a] >= 0) != (sol[b] >= 0)) {
        cerr << "ERROR: interdependency constraint #" << i << " violated:"
             << " job #" << a << (sol[a] >= 0 ? "runs" : "does not run")
             << " and job #" << b << (sol[b] >= 0 ? "runs" : "does not run")
             << endl;
        return 1;
      }
    }
