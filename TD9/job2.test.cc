...
int main() {
  ...
  const int kExpected[] = {2,  1,  10, 5,  8,  9,  10, 10, 10, 13,
                           28, -1, 32, 27, 23, 31, 34, -1, 48, 38};
  int num_jobs = 0;
  int num_machines = 0;
  ...
  for (const TestCase& t : test_cases) {
    ++tt;
    if (tt == 12 || tt == 18) continue;
    cout << endl
         << "==========================================================" << endl
         << "Testing '" << t.name << "'..." << endl;
    const vector<int> sol = BestJobAssignment2(t.jobs, t.machines, t.dep);
    ...
    for (int i = 0; i < t.exclu.size(); ++i) {
      ...
      for (int a : ex) {
        for (int b : ex) {
          if (a >= b) continue;
          if (sol[a] != -1 && sol[a] == sol[b]) {
            cerr << "ERROR: In your assignment, jobs #" << a << " and #" << b
                 << " are both assigned to machine #" << sol[a]
    ...
    // Verify that the dependency constraints are verified.
    for (int i = 0; i < t.dep.size(); ++i) {
      const int a = t.dep[i].first;
      const int b = t.dep[i].second;
      if (sol[a] >= 0 && sol[b] != sol[a]) {
        cerr << "ERROR: dependency constraint #" << i << " violated:"
             << " job #" << a << " runs on machine #" << sol[a] << " but job #"
             << b << " doesn't." << endl;
        return 1;
      }
    }
